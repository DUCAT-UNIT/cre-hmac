...
// Deterministic test-only scalar. This must never be copied from a deployed or
// historical production credential.
const (
...
func TestRevokedCredentialGuards(t *testing.T) {
	if err := ValidateSchnorrPubkeyNotRevoked(RevokedOracleSchnorrPubkey); err == nil {
		t.Fatal("expected the publicly disclosed oracle identity to be rejected")
	}

	kd, err := DeriveKeys(testPrivateKey)
	if err != nil {
		t.Fatalf("synthetic test key should remain valid: %v", err)
	}
	defer kd.Zero()
	if err := ValidateSchnorrPubkeyNotRevoked(kd.SchnorrPubkey); err != nil {
		t.Fatalf("synthetic test public key was rejected: %v", err)
	}

	if err := ValidateClientSecretNotRevoked([]byte("synthetic-test-client-secret")); err != nil {
		t.Fatalf("synthetic client secret was rejected: %v", err)
	}

	// Exercise the shared fingerprint matcher without embedding either
	// historical credential in the test suite.
	synthetic := []byte("fingerprint-test-value")
	fingerprint := sha256.Sum256(synthetic)
	if !matchesCredentialFingerprint(synthetic, fingerprint) {
		t.Fatal("matching credential fingerprint was not detected")
	}
	if matchesCredentialFingerprint([]byte("different-value"), fingerprint) {
		t.Fatal("non-matching credential fingerprint was reported as revoked")
	}
}
...
func TestCrossImplementationVectors(t *testing.T) {
	...
	const (
		...
		expectedCommitHash = "330c9b6aa9f773e99c13522176e775d473086be3f87001948a8c3f83f1ae1a2e"
		expectedTholdKey   = "734cc59144dba263dd80fa4f16cd1e086ef45eb22b4f1cc18b9100eca4721b9d"
		expectedTholdHash  = "d2551eb1cff86e60273e40a162ac6dbfb708c7b8"
		expectedContractID = "318837c924b3bbe7eff84c92336f56a0cafe46d921b3c102eac22950fdb88986"
	)
