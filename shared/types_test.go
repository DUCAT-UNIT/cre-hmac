...
func TestConfigValidate(t *testing.T) {
	...
	validConfig := Config{
		...
		ReportSigners:             []string{"0x1111111111111111111111111111111111111111"},
		ReportSignerThreshold:     1,
	...
	tests := []struct {
		...
		{"data_stream_url malformed", &Config{ClientID: "id", DataStreamURL: "x", FeedID: "feed", RelayURL: "wss://relay.example.com", Network: "net", AuthorizedKey: validAuthKey}, true, "absolute URL"},
		{"data_stream_url userinfo rejected", &Config{ClientID: "id", DataStreamURL: "https://user:pass@data.example.com", FeedID: "feed", RelayURL: "wss://relay.example.com", Network: "net", AuthorizedKey: validAuthKey}, true, "must not contain userinfo"},
		...
		{"relay_url localhost allowed", &Config{ClientID: "id", DataStreamURL: "http://localhost:8080", FeedID: "feed", RelayURL: "ws://localhost:7000", Network: "net", AuthorizedKey: validAuthKey}, false, ""},
		{"relay_url short input rejected without panic", &Config{ClientID: "id", DataStreamURL: "https://data.example.com", FeedID: "feed", RelayURL: "x", Network: "net", AuthorizedKey: validAuthKey}, true, "absolute URL"},
		{"relay_url loopback IPv6 allowed", &Config{ClientID: "id", DataStreamURL: "http://[::1]:8080", FeedID: "feed", RelayURL: "ws://[::1]:7000", Network: "net", AuthorizedKey: validAuthKey}, false, ""},
		{"report verification disabled externally", &Config{ClientID: "id", DataStreamURL: "https://data.example.com", FeedID: "feed", RelayURL: "wss://relay.example.com", Network: "net", AuthorizedKey: validAuthKey}, true, "require_report_verification must be true"},
...
func TestConfigValidateCronConfig(t *testing.T) {
	baseConfig := Config{
		ClientID:      "test-client",
		DataStreamURL: "http://localhost:8080",
		FeedID:        "feed-123",
		RelayURL:      "wss://relay.example.com",
		Network:       "mutiny",
	...
	tests := []struct {
		...
		{"valid cron config", "0 */5 * * * *", 1.36, 5.0, 0.05, false, ""},
...
func TestValidateCronConfigRejectsUnsafeLiquidationThreshold(t *testing.T) {
	base := Config{CronSchedule: "0 * * * * *", RateMin: 1.36, RateMax: 10, StepSize: 0.01}
	tests := []struct {
		name  string
		value float64
	}{
		{"negative", -1},
		{"not finite", math.Inf(1)},
		{"equal to minimum rate", 1.36},
		{"above minimum rate", 1.37},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cfg := base
			cfg.LiquidationThold = tt.value
			if err := cfg.ValidateCronConfig(); err == nil {
				t.Fatal("expected unsafe liquidation_thold to be rejected")
			}
		})
	}
}
...
func TestConfigJSONRejectsUnknownField(t *testing.T) {
	var cfg Config
	err := json.Unmarshal([]byte(`{"client_id":"x","require_report_verfication":true}`), &cfg)
	if err == nil || !strings.Contains(err.Error(), "unknown field") {
		t.Fatalf("expected unknown config field rejection, got %v", err)
	}
}

func TestValidateNoDuplicateObjectFields(t *testing.T) {
	tests := []struct {
		name    string
		input   string
		wantErr bool
	}{
		{"unique", `{"action":"adjust","pct":1}`, false},
		{"duplicate action", `{"action":"adjust","action":"status"}`, true},
		{"not object", `[]`, true},
		{"trailing value", `{} {}`, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := ValidateNoDuplicateObjectFields([]byte(tt.input))
			if (err != nil) != tt.wantErr {
				t.Fatalf("ValidateNoDuplicateObjectFields() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}

func TestConfigJSONRejectsDuplicateField(t *testing.T) {
	var cfg Config
	err := json.Unmarshal([]byte(`{"require_report_verification":true,"require_report_verification":false}`), &cfg)
	if err == nil || !strings.Contains(err.Error(), "duplicate JSON field") {
		t.Fatalf("expected duplicate config field rejection, got %v", err)
	}
}

func TestConfigJSONAcceptsDeploymentNonce(t *testing.T) {
	var cfg Config
	if err := json.Unmarshal([]byte(`{"deployment_nonce":"release-1"}`), &cfg); err != nil {
		t.Fatalf("expected deployment_nonce to be recognized: %v", err)
	}
	if cfg.DeploymentNonce != "release-1" {
		t.Fatalf("DeploymentNonce = %q, want release-1", cfg.DeploymentNonce)
	}
}

func TestValidateReportVerificationRejectsDuplicateSigners(t *testing.T) {
	cfg := Config{
		ReportSigners: []string{
			"0x1111111111111111111111111111111111111111",
			"0X1111111111111111111111111111111111111111",
		},
		ReportSignerThreshold:     2,
		RequireReportVerification: true,
	}
	if err := cfg.validateReportVerification(); err == nil || !strings.Contains(err.Error(), "duplicates") {
...
func TestEvaluateQuotesRequestValidate(t *testing.T) {
	validHash := strings.Repeat("a", 40)
	validCommitHash := strings.Repeat("c", 64)
	...
	tests := []struct {
		...
		{"single commit hash", &EvaluateQuotesRequest{TholdHashes: []string{validCommitHash}}, false, ""},
		{"mixed hash formats", &EvaluateQuotesRequest{TholdHashes: []string{validHash, validCommitHash}}, false, ""},
		{"duplicate hash", &EvaluateQuotesRequest{TholdHashes: []string{validHash, validHash}}, true, "duplicate hash"},
		{"multiple hashes", &EvaluateQuotesRequest{TholdHashes: validHashes}, false, ""},
		{"max hashes", &EvaluateQuotesRequest{TholdHashes: makeHashes(MaxBatchSize)}, false, ""},
		...
		// Too many
		{"too many hashes", &EvaluateQuotesRequest{TholdHashes: makeHashes(MaxBatchSize + 1)}, true, fmt.Sprintf("max %d", MaxBatchSize)},
		...
		{"invalid commit hash chars", &EvaluateQuotesRequest{TholdHashes: []string{strings.Repeat("g", 64)}}, true, "at index 0"},
		{"uppercase at index 2", &EvaluateQuotesRequest{TholdHashes: []string{validHash, strings.Repeat("b", 40), strings.Repeat("A", 40)}}, true, "at index 2"},
	}
...
func TestNostrEventHasTagValue(t *testing.T) {
	event := &NostrEvent{Tags: [][]string{{"h", "commit"}, {"h", "threshold"}, {"empty"}}}
	if !event.HasTagValue("h", "commit") || !event.HasTagValue("h", "threshold") {
		t.Fatal("expected exact h-tag values to be found")
	}
	if event.HasTagValue("h", "other") || event.HasTagValue("d", "commit") {
		t.Fatal("unexpected tag match")
	}
	if (*NostrEvent)(nil).HasTagValue("h", "commit") {
		t.Fatal("nil event must not match a tag")
	}
}
