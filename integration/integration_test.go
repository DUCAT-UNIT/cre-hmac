...
func TestHighVolumeContractCreation(t *testing.T) {
	...
	// Race instrumentation intentionally trades throughput for memory-safety
	// checks. Keep the correctness/load coverage above, but only enforce the
	// performance budget in an ordinary build where the result is meaningful.
	if raceDetectorEnabled {
		t.Log("Skipping throughput threshold under the race detector")
		return
	}
