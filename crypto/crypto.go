...
const (
	TagPriceCommitHash = "ducat/price_contract_commit"
	TagPriceContractID = "ducat/price_contract_id"

	// RevokedOracleSchnorrPubkey is the public identity derived from the oracle
	// private key that was committed to this repository's public history. Public
	// keys and SHA-256 fingerprints are safe to retain as deny-list identifiers;
	// the disclosed secret itself must never be reintroduced.
	RevokedOracleSchnorrPubkey = "6b5008a293291c14effeb0e8b7c56a80ecb5ca7b801768e17ec93092be6c0621"
)

var (
	// SHA-256 of the raw 32-byte revoked oracle private key. Keeping the
	// fingerprint lets startup reject the credential before it is used without
	// retaining the credential in source.
	revokedOraclePrivateKeySHA256 = [32]byte{
		0x8a, 0xe6, 0xb0, 0x82, 0xb4, 0x19, 0x4c, 0xb7,
		0xef, 0x31, 0xc0, 0x94, 0x83, 0x97, 0xc8, 0x8e,
		0xa5, 0x29, 0xf4, 0x5a, 0x6e, 0x3a, 0xc2, 0x17,
		0xf8, 0x00, 0x30, 0x37, 0xd1, 0xf9, 0x0b, 0x29,
	}

	// SHA-256 of the Chainlink client secret disclosed in the same incident.
	revokedChainlinkClientSecretSHA256 = [32]byte{
		0xae, 0x58, 0xa7, 0x3d, 0xf6, 0x9b, 0x47, 0xa9,
		0xfd, 0xe3, 0x7a, 0xa6, 0x04, 0x4b, 0xed, 0xdd,
		0x42, 0x6e, 0x30, 0x43, 0xe7, 0x04, 0xee, 0xa1,
		0x1f, 0xf4, 0xbe, 0x2f, 0xee, 0xf2, 0xd6, 0xa1,
	}
)
...
func matchesCredentialFingerprint(value []byte, fingerprint [32]byte) bool {
	digest := sha256.Sum256(value)
	return subtle.ConstantTimeCompare(digest[:], fingerprint[:]) == 1
}

// ValidateOraclePrivateKeyNotRevoked rejects the exact oracle private key that
// was publicly disclosed. The comparison is against a one-way fingerprint of
// the raw key bytes; no private credential is stored in the source tree.
func ValidateOraclePrivateKeyNotRevoked(privateKey []byte) error {
	if matchesCredentialFingerprint(privateKey, revokedOraclePrivateKeySHA256) {
		return fmt.Errorf("oracle private key is permanently revoked after public disclosure")
	}
	return nil
}

// ValidateClientSecretNotRevoked rejects the Chainlink client secret disclosed
// in public history. Rotation at the provider remains mandatory; this guard
// prevents an old vault value from being used by a newly deployed workflow.
func ValidateClientSecretNotRevoked(clientSecret []byte) error {
	if matchesCredentialFingerprint(clientSecret, revokedChainlinkClientSecretSHA256) {
		return fmt.Errorf("Chainlink client secret is permanently revoked after public disclosure")
	}
	return nil
}

// ValidateSchnorrPubkeyNotRevoked provides defense in depth for callers that
// already derived an oracle public key. Public-key comparison also catches the
// revoked identity if a future secret-loading path bypasses the raw-key check.
func ValidateSchnorrPubkeyNotRevoked(pubkey string) error {
	candidate, err := hex.DecodeString(pubkey)
	if err != nil {
		return fmt.Errorf("invalid Schnorr public key: %w", err)
	}
	revoked, err := hex.DecodeString(RevokedOracleSchnorrPubkey)
	if err != nil {
		return fmt.Errorf("invalid embedded revoked public key fingerprint: %w", err)
	}
	if len(candidate) == len(revoked) && subtle.ConstantTimeCompare(candidate, revoked) == 1 {
		return fmt.Errorf("oracle public key is permanently revoked after private-key disclosure")
	}
	return nil
}
...
func DeriveKeys(privateKeyHex string) (*KeyDerivation, error) {
	...
	if err := ValidateOraclePrivateKeyNotRevoked(privKeyBytes); err != nil {
		zeroBytes(privKeyBytes)
		return nil, err
	}
	...
	if err := ValidateSchnorrPubkeyNotRevoked(schnorrPubkey); err != nil {
		zeroBytes(privKeyBytes)
		return nil, err
	}
