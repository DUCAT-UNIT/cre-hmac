...
func (w *WorkflowSimulator) SimulateCheckQuote(domain string, tholdHash string) (*shared.NostrEvent, error) {
	...
	// Use w.Config.Network (original chain network) not SrvNetwork (mapped "test"/"main")
	// The commit hash was created with the original network name, not the mapped one
